    ...
    while not rospy.is_shutdown():
        if ser.in_waiting > 0:
            ...
            if (line == '1'):
                ...
                ser.write("Connection OK".encode('utf-8'))
                setOffboardMode()
            elif (line == '2'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Sending".encode('utf-8'))
                # sendImage()
            elif (line == '3'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Arming".encode('utf-8'))
                setArm()
            elif (line == '4'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Disarming".encode('utf-8'))
                setDisarm()
            elif (line == '5'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Take off".encode('utf-8'))
                setTakeoffMode()
            elif (line == '6'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Landing".encode('utf-8'))
                setLandMode()                                      
            elif (line == '7'):
                print(f'{timestamp} Data from PC: {line}')
                setHomePosition()
                ser.write("Setting home position".encode('utf-8'))
            elif (line == '8'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Uploading waypoint".encode('utf-8'))
                waypoint()
            elif (line == '9'):
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Start Waypoint".encode('utf-8'))
                startWaypoint()
            else:
                print(f'{timestamp} Data from PC: {line}')
                ser.write("Input not defined".encode('utf-8'))
